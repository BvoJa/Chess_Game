...
Playasblack::Playasblack() {
	...
	isWhiteTurn = 1; isGameEnd = _settings = 0;
	validMoves.clear();
	Black_history.clear();
	White_history.clear();
...
void Playasblack::PawnLogic(Figure A, vector<Vector2i>& move) {
	...
	if (isWhiteTurn) {
		...
		if (A.played == 0 && cord.y + 2 <= 7 && Occur[cord.x][cord.y + 2] == 0 && Occur[cord.x][cord.y + 1] == 0) {
	...
	if (A.played == 0 && cord.y >= 2 && Occur[cord.x][cord.y - 2] == 0 && Occur[cord.x][cord.y - 1] == 0) {
		move.push_back(Vector2i(cord.x, cord.y - 2));
	}
...
void Playasblack::Vertical(Figure A, vector<Vector2i>& move) {
	Vector2i cord = getCord(A);
	if (cord.x < 0 || cord.y < 0 || cord.x >= 9 || cord.y >= 9) return;
...
void Playasblack::drawPawnPromote() {
...
int Playasblack::PawnPromotion(Event event) {
...
void Playasblack::ResetAll() {
	...
	White_history.clear();
	countState.clear();
	square_from = square_to = { -1000, -1000 };
	criticalMove = 0;
...
long long Playasblack::getState() {
	long long state = 0;
...
array<int, 3> Playasblack::getAImove() {
	array<int, 3> Move;
	...
	if (Medium) {
		...
		for (int i = 0; i < 16; i++) {
			...
			Stockfox.UpdBoard(cord.y * 8 + cord.x, 0, White[i].type);
		}
		...
		for (int i = 0; i < 16; i++) {
			...
			Stockfox.UpdBoard(cord.y * 8 + cord.x, 1, Black[i].type);
		}

		Stockfox.upd_castle(0, king_main, White[4].played, 0);
		Stockfox.upd_castle(1, king_main, Black[4].played, 0);

		Stockfox.upd_castle(0, rook_main, White[7].played, 1);
		Stockfox.upd_castle(0, rook_main, White[0].played, 0);

		Stockfox.upd_castle(1, rook_main, Black[7].played, 1);
		Stockfox.upd_castle(1, rook_main, Black[0].played, 0);

		Stockfox.MakeMove();

		array<int, 4> res = Stockfox.getDecision(0);
		...
		for (int i = 0; i < 16; i++) {
			Vector2i cord = getCord(White[i]);
			if (cord.x == 7 - from.first && cord.y == 7 - from.second) {
				index = i;
				Move[0] = 7 - to.first;
				Move[1] = 7 - to.second;
				Move[2] = res[2];
		...
		cout << "AI in playsasblack return no moves !\n";
	}
	...
	if (Hard) {
		...
		array<int, 4> res = Stockfox.getDecision(1);
...
bool Playasblack::MakeMove(int x, int y, int promoted) {
	...
	if (isValidMove(White[index], AI) == 0) {
		cout << "DCM AI lo3\n";
		cout << "Index : " << index << '\n';
		cout << "Cordinate (x, y) : " << AI.x << ' ' << AI.y << ' ' << promoted << '\n';
		cout << "Type of the figure : " << White[index].type << '\n';
	...
	if (White[index].type == pawn_main && AI.y == 7) {
		int x = promoted;
		criticalMove = 0;
		sound[4].play();
...
void Playasblack::loadGame(const string& filename) {
	if (filename == "") return;
...
void Playasblack::SaveGame(const string& filename) {
	if (filename == "") return;
	ofstream fout(filename);

	fout << isWhiteTurn << ' ' << NumberOfMove << ' ' << criticalMove << " 1\n";
...
void Playasblack::PrintSaveFile() {
...
void Playasblack::PrintLoadFile() {
...
int Playasblack::handleEvents(RenderWindow& window) {
	...
	if (pawnPromote) {
		while (promotion.pollEvent(event)) {
			...
			UpdHistoryWhite(Black[index], target);
	...
	while (window.pollEvent(event)) {
		...
		if (event.type == Event::MouseButtonPressed && event.mouseButton.button == Mouse::Left) {
			if (pieceSelected == 0 && isWhiteTurn == 0) {
				for (int i = 0; i < 16; i++) {
					if (Black[i].shape.getGlobalBounds().contains(pos.x, pos.y)) {
						index = i;
						pieceSelected = 1;
						isDragging = 1;
						originalPosition = Black[i].shape.getPosition();
					}
				}
		...
		if (event.type == sf::Event::MouseButtonReleased && event.mouseButton.button == sf::Mouse::Left) {
			if (isDragging) {
				...
				if (MakeMove(pos.x, pos.y, 0) == 0)
					Blackdrag[index].shape.setPosition(originalPosition);
				else {
					for (int i = 0; i < 16; i++) {
						Blackdrag[i] = Black[i];
						Whitedrag[i] = White[i];
		...
		if (event.type == sf::Event::MouseMoved && isDragging) {
			Blackdrag[index].shape.setPosition(event.mouseMove.x - SIZE / 2, event.mouseMove.y - SIZE / 2);
		}
	}
	return Playasblack_main;
}
...
void Playasblack::UpdChanges(string path, char numberBoard) {
	string BoardPath = "Assets/images/BoardGame/rboard";
	BoardPath += numberBoard;
	BoardPath += ".png";
...
void Playasblack::draw(sf::RenderWindow& window) {
	Texture background;
	background.loadFromFile("Assets/images/rainbow.png");
