...
void TwoPlayerMode::drawPawnPromote() {
...
int TwoPlayerMode::PawnPromotion(Event event) {
...
void TwoPlayerMode::ResetAll() {
	...
	validMoves.clear();
	Black_history.clear();
	White_history.clear();	countState.clear();
	criticalMove = 0;
...
long long TwoPlayerMode::getState() {
	long long state = 0;
...
void TwoPlayerMode::loadGame(const string &filename) {
	if (filename == "") return; 
...
void TwoPlayerMode::SaveGame(const string &filename) {
	if (filename == "") return; 
...
bool TwoPlayerMode::makemove(int x, int y) {
	Vector2i temp;
	temp.x = x;
	temp.y = y;
	bool ok = 0; 
	...
	pieceSelected = pawnPromote = 0; 
	promotion.close(); 
	return ok; 
}

void TwoPlayerMode::PrintSaveFile() {
...
void TwoPlayerMode::PrintLoadFile() {
	...
	while (window.pollEvent(event)) {
		...
		if (event.type == sf::Event::MouseButtonReleased && event.mouseButton.button == sf::Mouse::Left) {
			if (isDragging) {
				...
				if (makemove(pos.x, pos.y) == 0) {
					if (isWhiteTurn) Whitedrag[index].shape.setPosition(originalPosition);
					else Blackdrag[index].shape.setPosition(originalPosition); 
				}
				else {
					for (int i = 0; i < 16; i++) {
						Blackdrag[i] = Black[i]; 
...
void TwoPlayerMode::UpdChanges(string path, char numberBoard) {
...
void TwoPlayerMode::draw(sf::RenderWindow& window) {
	Texture background; 
	background.loadFromFile("Assets/images/player.jpg");
	...
	if (isWhiteTurn) {
		...
		Indicator.setFillColor(sf::Color(138, 43, 226)); 
	...
	if (isGameEnd) {
		...
		if (isGameEnd == checkmate_main) {
			if (isWhiteTurn == 0) endText.setString("White Wins!");
			if (isWhiteTurn) endText.setString("Black Wins!");
		}
		else {
